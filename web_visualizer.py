...
class WebVisualizer:
    ...
    def create_dashboard(
        ...
        """Create an HTML dashboard summarizing detections and heuristic metrics."""
        ...
        disclaimer = html.escape(HEURISTIC_DISCLAIMER)
...
<body>
  ...
  <main>
    <section class="grid">
      ...
      {WebVisualizer._metric_card("Heuristic Score", health.get("overall_status", "Unknown"))}
      {WebVisualizer._metric_card("Score", f'{health.get("overall_health_score", 0)}/100')}
      ...
      {WebVisualizer._metric_card("High-intensity", f'{composition.get("school", 0):,}')}
    </section>

    <section class="card">
      <h2>Detection Map</h2>
      <p class="muted">Interactive map of heuristic intensity signatures (requires network for basemap tiles).</p>
      <div id="map" role="img" aria-label="Detection map"></div>
    </section>
  ...
  <script>
    ...
    const layer = L.geoJSON(geojsonData, {{
      pointToLayer: (feature, latlng) => {{
        ...
        const confidence = (feature.properties && feature.properties.confidence) || 0.5;
        const radius = 5 + Math.min(confidence, 1) * 8;
      ...
      onEachFeature: (feature, markerLayer) => {{
        ...
        markerLayer.bindPopup(
          `<strong>${{props.size || 'unknown'}}</strong><br>` +
          `Depth: ${{props.depth_m ?? 'n/a'}} m<br>` +
          `Intensity: ${{props.intensity ?? 'n/a'}}<br>` +
