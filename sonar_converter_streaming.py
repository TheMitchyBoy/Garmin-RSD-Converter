...
from sonar_schema import CSV_COLUMNS

logger = logging.getLogger(__name__)

_PARSE_ERRORS = (struct.error, IndexError, ValueError, TypeError)
    ...
    def parse_to_csv(self, input_file: Path, output_file: Path, stride: int = 256) -> int:
        ...
        try:
            with open(input_file, 'rb') as infile, \
                ...
                writer = csv.DictWriter(outfile, fieldnames=list(CSV_COLUMNS))
                writer.writeheader()
                ...
                while offset < file_size - 256:
                    try:
                        ...
                        logger.debug(f"Error at offset {offset}: {exc}")
            ...
            logger.error(f"Error during parsing: {exc}")
            raise
